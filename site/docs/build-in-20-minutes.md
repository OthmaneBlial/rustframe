...
Windows users can run the release's `rustframe-cli-installer.ps1`. `cargo install --git https://github.com/OthmaneBlial/rustframe --tag v0.1.0-rc.3 rustframe-cli --locked` remains the build-from-source alternative. The initial `rustframe-api` npm publication must be complete before the registry-only project install below can succeed.
